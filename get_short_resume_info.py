...
def get_resume_info11(resume_bytes):
    """获得PDF简历文本（只有文本块）"""
    docs = fitz.open("pdf",resume_bytes)
    content = []
    for doc in docs:
...
def get_resume_info3(resume_content2,answers):
    ...
    system_text = """
       ...
       生成一份结构化「职业画像」。
       该职业画像将用于后续：推荐适合的岗位检索词、判断用户与岗位的匹配程度。
...
def get_resume_info4(resume_content3):
    ...
    system_text="""
    # 角色
    你是一名资深招聘顾问和职业规划专家，熟悉Boss直聘、智联招聘、猎聘等招聘平台的岗位命名规则以及企业招聘习惯。
    ...
    ### 一、核心岗位关键词
    推荐数量不少于5个，但也不超过10个。
    例如：产品经理、高级产品经理、产品运营、运营产品经理、AI产品经理
    ......
    ### 二、岗位同义词
    推荐数量不少于5个，但也不超过10个。
    ...
    ### 三、行业关键词
    推荐数量不少于5个，但也不超过10个。
    例如：互联网、SaaS、企业服务、金融科技、AI
    ......
    ### 四、技能关键词
    推荐数量不少于5个，但也不超过10个。
    例如：Python、SQL、Excel、Axure、PRD
    ......
    ### 五、岗位+行业组合推荐
    推荐数量不少于5个，但也不超过10个。
    ...
    例如：
    - AI 产品经理 ★★★★★
    - ("AI产品经理" OR "产品经理") AND ("LLM" OR "大模型") ★★★★★
    - SQL ★★★★☆
    - ("数据分析师" OR BI) AND SQL AND Python ★★★★☆
    - 医疗 产品经理 ★★★☆☆
    
    # 注意
    优先采用招聘网站真实出现频率高的岗位名称，而不是理论名称。
    禁止回答除输出格式以外的内容，禁止显示用户姓名。
    下面是求职者画像：
    {user_portrait}
    """
...
def get_resume_info6(user_input,portrait):
    ...
    如果 {portrait} 为空、缺失、内容不足，说明用户还没有完成职业画像。
    ...
    等职业画像生成后，本喵才能根据你的技能、经历和目标，帮你筛选最适合的岗位！”
...
def get_resume_info7():
    ...
    resume_content3=get_resume_info3(resume_content2,answers)
