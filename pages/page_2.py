...
uploaded_file = st.file_uploader("请上传之前爬虫喵捕获的岗位信息", type=["json"])
...
analysis_choice = st.radio("模式选择",
                           ["普通模式（该模式，对全部岗位进行评分，无需配置向量库）",
                            "Rag 模式（该模式，仅分析匹配度最高的 Top50 岗位，需要配置向量库）"],
...
if st.session_state.new_job_infos:
    ...
    with colu2:
        ...
        st.download_button(
            label="📥 下载岗位分析结果(Excel)",
            data=buffer.getvalue(),
            file_name="岗位分析结果.xlsx",
