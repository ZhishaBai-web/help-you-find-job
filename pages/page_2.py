...
json_profile = st.text_area("",placeholder="请将工作喵输出的JSON画像内容复制到此处",label_visibility="collapsed")
...
if submit:
    ...
    if "Rag分析" in analysis_choice:
        ...
        if uploaded_file:
            ...
            if len(datas) < 50:
                errors.append(f"- 岗位 JSON 文件内岗位条数为{len(datas)}条，小于50条，不符合Rag条件")
...
if st.session_state.new_job_infos:
    ...
    with colu1:
        with st.spinner("JSON文件正在生成，请稍等..."):
