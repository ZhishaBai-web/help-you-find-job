...
from get_short_resume_info import get_resume_info11
...
uploaded_file = st.file_uploader("上传pdf简历（非图片pdf），让工作喵帮您发现职业潜力 ✨", type=["pdf"])
file_bytes = False
if uploaded_file:
...
with tab1:
    if file_bytes and st.session_state.resume_content is None:
        with st.spinner("简历正在扫码，请稍等..."):
            st.session_state.resume_content = get_resume_info11(file_bytes)
            st.markdown('<div>', unsafe_allow_html=True)
...
with tab2:
    if st.session_state.resume_content and st.session_state.resume_refine is None:
        with st.spinner("简历正在精炼，请稍等..."):
            if st.session_state.resume_content=="请改为文本pdf，而不是直接图片转pdf":
                st.session_state.resume_refine = "请改为文本pdf，而不是直接图片转pdf"
            else:
